...
class CardCheck(object):
    ...
    SHACKLES = {'zap': Card("Paus", "14"),
                'hearts': Card("Copas", "13"),
                'diamonds': Card("Ouros", "11"),
                'spades': Card("Espadas", "12")
                }
    ...
    def check_if_is_shackle(self, card):
        ...
        spades = card.value is self.SHACKLES['spades'].value and card.suit is self.SHACKLES['spades'].suit

        if zap or hearts or diamonds or spades:
            is_shackle = True
        else:
            is_shackle = False
